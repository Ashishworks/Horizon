  ...
  useEffect(() => {
    const generateInsight = async () => {
      ...
      const { data, error } = await supabase
        .from('journals')
        .select('mood, sleep_hours, exercise, screen_work, screen_entertainment, stress_level')
        .eq('user_id', user.id)
        .gte('date', fromDate);
      ...
      /* ---------- Exercise vs Mood ---------- */
      const exerciseDays = data.filter(d => d.exercise && d.exercise.length > 0);
      const noExerciseDays = data.filter(d => !d.exercise || d.exercise.length === 0);
